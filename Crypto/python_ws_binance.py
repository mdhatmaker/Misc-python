...
def onConnectError(socket, error):
    logging.info("on connect error got called: %s" % error)
...
if __name__ == "__main__":
    symbol = "bnbbtc"
    #socket = Socketcluster.socket("wss://stream.binance.com:9443/ws/%s@depth" % symbol)
    socket = Socketcluster.socket("wss://stream.binance.com:9443/ws/%s@ticker" % symbol)
