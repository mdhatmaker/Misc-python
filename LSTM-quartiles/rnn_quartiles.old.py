...
print("Done.  vocab_size={0}".format(vocab_size))

#training_data = np.array([int(b,2) for b in training_data])
#dictionary = list(set(training_data))
#vocab_size = len(dictionary)
...
with tf.Session() as session:
    ...
    if load_model:
        ...
        while step < training_iters:
            ...
            #symbols_in = training_data[offset:offset+n_input]
            #symbols_in = np.reshape(np.array(symbols_in), [-1, n_input, 1])
            symbols_out_onehot = np.zeros([vocab_size], dtype=float)
            symbols_out_onehot[dictionary[str(training_data[offset+n_input])]] = 1.0
            symbols_out_onehot = np.reshape(symbols_out_onehot,[1,-1])
            #symbols_out = training_data[offset+n_input]
            #symbols_out = np.reshape(np.array(symbols_out), [1,-1])
            _, acc, loss, onehot_pred = session.run([optimizer, accuracy, cost, pred], \
                                                    feed_dict={x: symbols_in_keys, y: symbols_out_onehot})
            #_, acc, loss, quartile_pred = session.run([optimizer, accuracy, cost, pred], \
            ...
            if (step+1) % display_step == 0:
                ...
                symbols_out = training_data[offset + n_input]
                #symbols_out_pred = quartile_pred
